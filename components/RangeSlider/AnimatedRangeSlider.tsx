...
export default function AnimatedRangeSlider({ showComponent }: { showComponent: boolean }) {
  ...
  useEffect(() => {
    // Función para animar el aumento de valor
    const animateValue = () => {
      const targetValue = 2.5; // Valor máximo
      const step = 0.1; // Incremento por paso      

      if (sliderValue < targetValue) {
        // Si el valor actual es menor que el valor máximo, aumenta el valor
        setSliderValue((prevValue) => Math.min(prevValue + step, targetValue));
      }
    };

    // Inicia la animación en un intervalo de tiempo
    const animationInterval = setInterval(animateValue, 30);
